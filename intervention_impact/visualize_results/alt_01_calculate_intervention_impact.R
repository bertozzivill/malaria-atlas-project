...
compare_summary <- compare[, lapply(.SD, mean), by = .(Site_Name, day, ITN_Coverage, ITN_Retention_Halflife, ITN_Initial_Block, ITN_Initial_Kill, x_Temporary_Larval_Habitat), 
...
block_to_plot <- c(0.6, 0.9)
x_temps_to_plot <- c(0.1995, 0.5012, 5.0119)

subset <- compare[day==day_to_plot & Site_Name %in% sites_to_plot & ITN_Initial_Kill %in% kill_to_plot & ITN_Retention_Halflife %in% ret_to_plot & ITN_Initial_Block %in% block_to_plot]
subset_summary <- compare_summary[day==day_to_plot & Site_Name %in% sites_to_plot & ITN_Initial_Kill %in% kill_to_plot & ITN_Retention_Halflife %in% ret_to_plot & ITN_Initial_Block %in% block_to_plot] 


line_plots <- ggplot(subset, aes(x=control_inc, y=final_inc, color=factor(ITN_Retention_Halflife))) + 
                  geom_vline(data=unique(subset_summary[x_Temporary_Larval_Habitat %in% x_temps_to_plot, 
                                                        list(Site_Name, x_Temporary_Larval_Habitat, control_inc)]), 
