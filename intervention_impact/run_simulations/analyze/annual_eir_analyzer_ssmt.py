...
class EIRAnalyzer(BaseAnalyzer):
    ...
    def __init__(self, dir_name, report_names=["AnnualAverage"], sweep_variables=None, working_dir=".",
                 last_year_only=True):
        super(EIRAnalyzer, self).__init__(working_dir=working_dir,
    ...
    def select_simulation_data(self, data, simulation):
        colname = "initial_eir" if self.dir_name == "burnin" else "final_eir"
        ...
        for site_name in self.sitenames:
            ...
            try:
                channeldata = data["output/MalariaSummaryReport_{name}.json".format(name=site_name)]["DataByTime"]["Annual EIR"]
